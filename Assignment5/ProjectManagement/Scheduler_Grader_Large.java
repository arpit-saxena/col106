...
//        }
    ...
    static void execute() throws IOException {
        ...
        ArrayList<String> list_projects = new ArrayList<>();

        /**
         * Users creation
         */
        for (int i = 0; i < MAX_QUERIES; i++) {
            list_users.add("User" + i);
            cmd = gen_user_query("User" + i).split(" ");
            handle_cmd_query(cmd);
        }

        for (int i = 0; i < MAX_QUERIES; i++) {
            list_projects.add("Project" + i);
        ...
        for (int i = 0; i < MAX_QUERIES; i++) {
            cmd = (new String("NEW_FLUSH 100")).split(" ");
            handle_cmd_query(cmd);
        }
        for (String project : list_projects) {
            cmd = (new String("NEW_PROJECT " + project + " 1 100")).split(" ");
            handle_cmd_query(cmd);
        }

        for (String user : list_users) {
            cmd = (new String("NEW_USER " + user + " 1 100")).split(" ");
            handle_cmd_query(cmd);
        }

        for (String project : list_projects) {
            for (String user : list_users) {
                cmd = (new String("NEW_PROJECTUSER " + project + " " + user + " 1 100")).split(" ");
                handle_cmd_query(cmd);
            }
        }
        for (int i = 0; i < MAX_QUERIES; i++) {
            cmd = (new String("NEW_PRIORITY " + i)).split(" ");
            handle_cmd_query(cmd);
        }
        for (int i = 0; i < MAX_QUERIES; i++) {
            cmd = (new String("NEW_TOP " + i)).split(" ");
            handle_cmd_query(cmd);
        }
